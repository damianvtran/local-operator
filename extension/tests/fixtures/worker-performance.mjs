...
export function installWorkerPerformance({ port, id, token, onState = () => {}, action = async () => {} }) {
  ...
  globalThis.chrome = {
    ...
    runtime: { id, getManifest: () => ({ version: "0.1.20" }), getURL: path => `chrome-extension://${id}/${path}`, sendMessage: async () => {}, onStartup: event("startup"), onInstalled: event("installed"), onMessage: event("message"), openOptionsPage() {} },
  };
