...
async def run_tui(
    ...
    # `terminal` theme only when the terminal answered. Never fatal: a theme is
    # not worth a boot.
    from local_operator.tui import host_theme

    try:
        host_theme.install(theme_name)
    except Exception:
        logger.debug("terminal theme probe failed", exc_info=True)
