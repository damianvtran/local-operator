...
def _read_restored_todos(transcript_directory: str) -> list[dict[str, Any]]:
    ...
    A PURE READ of somebody else's session that must leave nothing behind, and
    the reader enforces that by construction: ``read_latest_custom`` opens the
    journal read-only and never touches — let alone creates — the directory.
    ``Transcript`` used to be borrowed for the same guarantee via
    ``defer_materialise``, because it mkdirs its directory on construction, so
    merely opening a child's page created an empty session directory in the real
    store whenever the path did not exist — and the path can now be DERIVED from
    a ``session_id`` (see ``frontend_state._snapshot_session_dir``), so a remote
    follower whose children live on another machine would litter this one with a
    phantom directory per page open (review round 1, M1). The reader also walks
    only the tail, where ``Transcript`` decoded the child's whole journal to
    answer this one question.
    """
    from local_operator.session.transcript import read_latest_custom

    details = read_latest_custom(transcript_directory, "todo_snapshot") or {}
