...
- ``build()`` embeds ``"<name>: <description>"`` per skill into one faiss
  ``IndexFlatIP`` built ONCE and reused for every ``select()`` (vectors are
  L2-normalized, so inner product == cosine).
- Vectors persist at ``cache_dir/<identity>.skills.npz`` with a matching
  ...
  content hash is ALSO embedded inside the npz and re-verified after load —
  meta and vectors can only ever describe each other.
...
def _content_hash(skills: Sequence[Skill]) -> str:
    ...
    Covers ``(name, description, file_path, mtime)`` in LIST ORDER: faiss
...
class SkillIndex:
    ...
    The index is positional: faiss row ``i`` corresponds to
    ...
    def __init__(
        ...
        self._faiss_index = None  # built once lazily, reused per select
    ...
    def _cache_vectors_path(self) -> Path:
        return self.cache_dir / f"{self._cache_stem()}.npz"
    ...
    async def build(self, backend: EmbeddingBackend | None = None) -> None:
        ...
        if not self.skills:
            self._matrix = np.zeros((0, max(backend.dim, 1)), dtype=np.float32)
    ...
    def _try_load_cache(self, content_hash: str) -> bool:
        ...
        try:
            with np.load(vectors_path) as data:
                matrix = data.get("vectors")
                stored_hash = data.get("content_hash")
        except Exception:  # noqa: BLE001 — truncated/corrupt npz → rebuild
            return False
        if matrix is None:
            return False
        # Hash embedded in the npz itself: meta and vectors can only ever
        # describe each other, never an interleaved neighbor's payload.
        if stored_hash is None or str(stored_hash) != content_hash:
            return False
        recorded_dim = meta.get("dim")
        if matrix.ndim != 2 or recorded_dim != matrix.shape[1]:
            return False
        if matrix.shape != (len(self.skills), recorded_dim):
            return False
        ...
        if declared and matrix.shape[1] != declared:
            return False
    ...
    def _persist_cache(self, content_hash: str, matrix: np.ndarray) -> None:
        ...
        in-npz hash is the final cross-check on load).
        """
        try:
    ...
    async def select(self, query: str, k: int = 8, threshold: float | None = None) -> list[Skill]:
        ...
        built ONCE and reused). ``threshold`` defaults to the backend's
    ...
    def _scores(self, query_vec: list[float]) -> list[float]:
        ...
        Uses the single faiss ``IndexFlatIP`` (built once, kept for the
        index's lifetime — no per-turn re-add); faiss returns rows sorted by
        ``ids`` naming the original matrix row, and we scatter back into a
        list indexed by skill position so the caller can filter by threshold
        and hide in one pass.
        """
        import faiss  # imported lazily: faiss-cpu is a heavy native dep
        ...
        distances, ids = self._faiss_index.search(np.ascontiguousarray(q), self._faiss_index.ntotal)
        scores = [0.0] * len(self.skills)
        for rank in range(len(ids[0])):
            row = int(ids[0][rank])
            if row < 0:
                continue
            scores[row] = float(distances[0][rank])
        return scores
