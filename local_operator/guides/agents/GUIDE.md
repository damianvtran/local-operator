...
- `reset` — the restore verb. Puts the packaged starter back over a role that was INSTALLED from that starter and then edited, and prints every field it replaced (instructions, `tools`, `effort`, `delegate`) so the overwrite is recoverable by copy-paste. It restores on provenance, not on name: a role you authored yourself is refused rather than overwritten, including when you authored it under a starter's name such as `scout`. Roles installed before provenance was recorded read as self-authored and are refused too, which errs toward keeping your work; re-run `install` on a fresh name if you want a packaged copy to compare against.
