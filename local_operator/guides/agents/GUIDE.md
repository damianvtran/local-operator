...
- has its model decided when it starts or resumes. An inheriting child takes the parent's model at that moment; a pinned child (an `effort` tier, or a role's own pin) re-resolves its tier from current config. So `hub op='resume'` does NOT bring a child back on the model it ran on before: after the parent switched models, a resumed inheriting child runs on the new one. The resume receipt names the model the same way the launch result does, and adds `(its previous run was on <provider/model>)` when it changed. Switching the parent — `/model`, the phone, or another session — does not change children already running; pause and resume one to move it.
