...
- has its model decided when it starts or resumes. An inheriting child takes its OWN parent's model at that moment — the session that launched it, so a worker of a manager pinned to a cheaper tier resumes on the manager's model, not on yours. A pinned child (an `effort` tier, or a role's own pin) re-resolves its tier from current config. So `hub op='resume'` does NOT bring a child back on the model it ran on before: after its parent switched models, a resumed inheriting child runs on the new one. The resume receipt names the model the same way the launch result does (`on its parent's model (<provider/model>)` for a manager's worker), and adds `(its previous run was on <provider/model>)` when it changed. In the one case where a nested child's parent model can no longer be found (its record is gone), it keeps its previous model and the receipt says so. Switching a parent's model — its own `/model`, or a switch sent from the phone or `lop attach` — does not change children already running; pause and resume one to move it.
