...
- `retry`: `enabled`, `maxRetries`, `baseDelayMs`, `modelFallback`, `usageAwareFallback`, `usageReservePercent`, `usageAwareAccountPick` (default `true`: a session's first pick among same-provider accounts prefers the one with the most remaining quota in the cached `/usage` report; `false` restores the pure per-session hash spread for sessions — see the failover guide), and `fallbackChains` (snake_case spellings are also accepted for retry fields)
