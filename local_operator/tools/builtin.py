...
def build_ask_tool(context: ToolContext) -> AgentTool | None:
    ...
    return AgentTool(
        ...
        description=(
            ...
            "chose. Use this only when the action is destructive or irreversible and "
            "not already authorized, when the REQUEST ITSELF has two plausible readings "
            "that send the work in different directions and no evidence picks between "
            "them, or when you need something only the user has (a credential, an "
            "access decision). Two technical approaches is not ambiguity: weigh them, "
            "pick one, and say why. Work "
            "the user already asked for is authorized: do not stop to confirm it, "
            "re-ask what the conversation answered, or seek permission to continue. "
