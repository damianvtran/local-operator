...
#: here because it is the thing most likely to be violated by accident:
...
#: commands — never a silently wrong answer. Additive optional fields, new
#: ``ErrorCode``s the peer only emits, and new events an old peer harmlessly
#: drops all keep the floor where it is.
...
#: The first extension release that carried the ``owner_*`` ownership lifecycle
#: (PR #798, ``ee146fb73`` — the release that first shipped ``owner_*``).
#:
...
OWNERSHIP_MIN_EXTENSION_VERSION = "0.1.10"
