...
* tool-call ARGUMENTS: 69,038 calls, p50 328 chars, p99 9,652 — **no bound of any
  kind**, largest observed single call 130,491 chars. That is 34% of all message
  bytes in the store, versus 59% for results and 6% for everything else.

So this is a BACKSTOP, not a latency lever: at the default bound it removes
~3.6% of the replayed message chars (measured, same sample). Latency-wise that
is single-digit milliseconds at the provider's measured ~5 ms per 1k tokens. It
earns its place by closing two unbounded paths — one of them entirely uncapped —
so that no single row can dominate a session's every turn. Tightening the bound
is a one-line, measured, deferred decision (see the PR body) and it is decidedly
NOT taken by default: at 2,048 chars it would remove 41.5% of replayed message
chars (~94k tokens off a measured 227k-token prompt, ~490 ms of TTFB) at the cost
of eliding the middle of most tool results and long argument values, which is a
lossy global behaviour change and belongs to the operator rather than to this
module's default.
