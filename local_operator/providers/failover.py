...
def is_invalidated_credential_error(error: BaseException) -> bool:
    """The token itself is dead (revoked/expired/invalid), not just denied."""
    if not isinstance(error, ProviderError):
        return False
    if error.status != 401:
        return False
    lowered = error.message.lower()
    return any(marker in lowered for marker in ("invalid", "revoked", "expired", "unauthorized"))
...
async def resolve_next_key(
    ...
    *,
    ...
    Termination: key already attempted, ≥64 attempts, resolver gives up, or
    the signal aborted.
    """
    ...
    if not state.refreshed_current and not is_direct_credential_rotation_error(error):
...
async def stream_with_failover(
    ...
    *,
    ...
    Credential rotation happens inside the current provider; when that is
    exhausted, the model fallback chain is walked. Events are forwarded as
    they arrive — once anything has been yielded, a mid-stream failure is
    re-raised (partial output cannot be replayed). Raises
    :class:`ProviderError` with the last failure when every option is spent.
    """
    ...
    for selector in selectors:
        ...
        while state.attempts <= AUTH_RETRY_MAX_ATTEMPTS:
            key = await _resolve_key_for_provider(auth, provider, session_id, state, error)
            if key is None and error is not None:
                break  # rotation exhausted for this provider
            if key is None and not await _provider_allows_missing(auth, provider):
            ...
            forwarded_any = False
            try:
                async for event in client.stream(current_request, key):
                    forwarded_any = True
                    yield event
                ...
                if not retry.enabled:
                    raise
                if exc.auth_error or exc.status in (401, 403):
                    # Rotate immediately — refreshing cannot fix auth errors.
                    auth.rotate_sibling(provider, session_id, exc, api_key=key)
                    continue
                if exc.retryable:
                    delay = max(exc.retry_after_ms or 0, backoff_delay_ms(retry.base_delay_ms, state.attempts, rng=rng))
                    await asyncio.sleep(delay / 1000)
                    # Retry same selector with the next resolved key.
                    continue
                ...
                await asyncio.sleep(backoff_delay_ms(retry.base_delay_ms, state.attempts, rng=rng) / 1000)
                continue
...
async def _resolve_key_for_provider(
    auth: Any,
    provider: str,
    session_id: str | None,
    state: AuthRetryKeyState,
    ...
    """Bridge AuthStore into the a/b/c resolver shape."""
    ...
    return await resolve_next_key(state, resolver, error)
