...
def test_the_re_arm_fence_reads_a_position_and_never_an_existence(tmp_path: Path) -> None:
    ...
    The boundary around it, which a real runtime cannot be asked to produce to order: no
    ...
    WHY NO CELL HERE DRIVES A REAL FIRE FOR THIS FENCE, which is a deliberate gap rather
    than an oversight. A live-fire child was written and worked (it armed the shipped
    bound, took a real C-timer fire, re-armed through ``beat`` and disarmed), and its
    before/after artefact pair is on the PR against a real ``lop sessions``; but it hung
    about one run in twelve on a loaded host, parked in ``_arm_timer`` from
    ``_record_held_fire``'s ``_rearm`` — the arm-while-firing deadlock that PR #1517
    (``test/stall-watchdog-arm-deadlock-repro``, "the stall bound fires in Python, so no
    arm can park holding the GIL") exists to remove. A new cell must not
    put a known product defect into CI, so the live state stays evidenced on the PR and
    this cell pins the fence itself.
    """
    logs = tmp_path / "logs"
