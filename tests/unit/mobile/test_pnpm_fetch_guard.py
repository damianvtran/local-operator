...
refusal. This file pins the delta on top of those shapes, and re-tests none of them:
  ...
  * the child environment every package-manager command runs with
    (:func:`install._package_manager_env`) — the setting that makes the fetch
    unreachable, and the shared homes a seeded machine resolves against;
  * the SEEDED ARM (:func:`install._seeded_pnpm` / :func:`install._seeded_or_path`)
    — a pin already installed where pnpm keeps managed versions is used, VERIFIED
    by asking it, instead of refusing a machine that can already build;
...
THE ONE PLACE THIS DIFFERS FROM #1394's GUARD, stated because two guards for one
hazard disagreeing silently is worse than either policy: main refuses whenever the
runner's reported version is not the pin. This prefers, before refusing and after
corepack, a pin that is already on the disk — so on a host with a seeded 11.22.0 and
a PATH 10.30.3, main refuses and this builds. The trade-off is argued in the PR
body; the boundary is pinned here, and the permissive arm cannot fetch anything:
:func:`install._seeded_pnpm` requires the candidate to ANSWER with the pin, through a
probe that runs in an empty directory with the fetch disarmed.
...
def test_only_an_exact_pnpm_pin_disarms_the_fetch(
    tmp_path: Path, pin: str | None, expected: bool
) -> None:
    """A pin the fetch can act on is disarmed; a range is left alone.

    pnpm reaches its self-install only for a version ``semver.valid`` accepts, so a
    range is not a fetch and the setting would turn pnpm's warn-and-continue into an
    error on a tree that works today.
    """
    web = _web(tmp_path, pin=pin)

    armed = install._MANAGE_PM_VERSIONS_ENV in install._package_manager_env(web)
...
def test_the_seeded_pin_builds_where_the_path_pnpm_would_be_refused(
    ...
    prose. With :func:`install._seeded_or_path`'s call removed this test fails with
    the refusal in hand (measured — see the PR body).
    """
    home = tmp_path / "pnpm-home"
    monkeypatch.setenv("PNPM_HOME", str(home))
    ...
    # The candidate's verification probe, then the guard's own on the chosen runner,
    # then the two steps — counted rather than described, because a boundary that
    # spawns more than it says is the default this module was written about.
...
def test_a_matching_path_runner_is_not_replaced_by_a_seeded_pin(
    ...
    """The seeded arm is a fallback, not a preference for its own copy.

    PATH is what the operator chose; switching away from a runner that already
    satisfies the pin would be a change for nothing, and it would hide a broken
    install behind a working one.
    """
    home = tmp_path / "pnpm-home"
    monkeypatch.setenv("PNPM_HOME", str(home))
    seeded = _seed(home, "11.22.0", version="11.22.0", exit=0, dist=True)
    ...
    assert seeded.argv_seen() == [["--version"]], "the candidate is verified and no more"
...
def test_a_corepack_shaped_runner_is_left_alone_even_with_a_seeded_pin(
    ...
    assert install._seeded_or_path(["pnpm"], web) == ["pnpm"]


def test_without_a_seeded_pin_the_mismatch_is_still_refused(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The boundary this delta must not weaken: nothing seeded, nothing changes.

    The refusal's copy, its routes and its probe count are
    ``test_bundle_build_bound.py``'s business (it owns that sentence); what is
    asserted here is only the shape this arm could have broken — with no candidate to
    prefer, the runner is handed on untouched and the guard still refuses.
    """
    monkeypatch.setenv("PNPM_HOME", str(tmp_path / "empty-home"))
    fake = StandIn(tmp_path / "bin", version="10.30.3", exit=1)
    web = _web(tmp_path, pin="pnpm@11.22.0")

    assert install._seeded_or_path(fake.runner, web) == fake.runner
