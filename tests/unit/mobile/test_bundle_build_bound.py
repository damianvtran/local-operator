...
def test_a_fast_allocator_is_killed_at_the_ceiling_not_by_the_clock(
    ...
    # 0.9, not the ceiling itself: this sampler reads the group BETWEEN the step's
    # own ticks, so the last reading it can land before the kill is one tick short of
    # the value the guard acted on. The floor is here to prove the child really
    # allocated what the reading assumes, not to re-derive the guard's arithmetic.
    assert peak >= int(110 * 1024 * 1024 * 0.9), (
        f"this test's own sampling never saw the group near the ceiling ({peak} B): "
        "the stand-in did not allocate what the reading assumes"
    )
