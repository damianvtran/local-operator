...
#: The real checkout, for the guards that assert a property over this tree.
...
def test_a_literal_name_import_in_a_hub_selects_the_module_it_names(tmp_path):
    ...
    _write(
        ...
        def load(key):
            return importlib.import_module(f"local_operator.providers.{key}")
        """,
    )
...
def test_the_report_names_what_stopped_the_selection(tmp_path):
    """A barrier that is not printed is the quiet narrowing this design forbids.

    One computed import in the tree is enough: the reference model cannot say what
    `importlib.import_module(name)` loads, so no selection of the test tree can be
    proven complete and the job runs whole-tree — with the site that stopped it
    named in the line a reader sees (#1322, decision item 2).
    """
    root = _fixture_repo(tmp_path)
    _write(
        root,
    ...
    assert plan.unresolved and "local_operator/agents.py" in plan.unresolved[0]
    report = "\n".join(plan.report())
    assert plan.decisions["test"].whole_tree
    assert "cannot place" in report
    assert "proven complete" in report
    assert "local_operator/agents.py" in report
...
def test_the_report_says_nothing_about_limits_when_no_test_selection_was_narrowed(tmp_path):
    ...
    assert plan.decisions["test"].whole_tree
    assert "reference model that reads imports" not in "\n".join(plan.report())


def test_a_narrowed_test_run_states_what_its_reference_model_cannot_see(tmp_path):
    """The other half: when a selection IS narrowed, its blind spots are printed.

    The model reads imports, literal path and module names, and directory scans.
    It cannot see dynamic attribute access, `eval`/`exec`, or a path built from
    data at run time, so a narrowed run is evidence about the files it ran and
    never about the tree — stated on the run, not implied.
    """
    root = _fixture_repo(tmp_path)  # no unplaceable reference anywhere in it
    plan = ci_scope.scope_plan(["test"], ["local_operator/alpha.py"], root)

    assert not plan.decisions["test"].whole_tree, plan.decisions["test"].notes
    assert not plan.unresolved
    report = "\n".join(plan.report())
    assert "reference model that reads imports" in report, report
    assert "CI remains the authoritative run" in report
...
def test_a_scan_this_graph_cannot_place_stops_the_selection(tmp_path):
    """A scan over a directory the graph cannot resolve is a reference it cannot read.

    The directory here is a `tmp_path` value: no literal, no `__file__`. The
    resolver therefore does not know what it reads, so it can neither select the
    reader nor rule it out — and a selection that cannot be proven complete is not
    made (#1322, decision item 2). Every earlier policy for this case was measured
    to be wrong in one direction or the other: arming every unplaced scan selected
    654 of 724 tests for ANY change, and arming only the ones that spell `.py`
    still let rounds 3 and 4 through.
    """
    root = _fixture_repo(tmp_path)
    _write(
        root,
    ...
    assert any("test_unplaced_scan.py" in site for site in plan.unresolved), plan.unresolved
    assert plan.decisions["test"].whole_tree
    assert "cannot place" in "\n".join(plan.report())
...
_SCAN_SHAPES: dict[str, tuple[str, dict[str, str], str, tuple[str, ...], tuple[str, ...]]] = {
    ...
    # Round 4's shapes: `Path(ROOT, "scripts")` used to resolve to the repository
    # ROOT (every argument is a path component, so they compose), and an
    # argument-less `.iterdir()` used to be dropped entirely — 158 scans invisible,
    # neither placed nor recorded.
    "path-multi-arg": (
        'for path in Path(ROOT, "scripts").glob("*.py"):\n    path.read_text()\n',
        {},
        "scripts/tool.py",
        ("tests/unit/test_scan_shape.py",),
        (),
...
def test_a_scan_shape_places_the_directory_it_reads(tmp_path, shape):
    """Each receiver shape the graph must place, and the one it must refuse."""
    ...
    if shape == "unplaceable-py":
        # A receiver the evaluator cannot place is a reference the model cannot
        # resolve, so NOTHING in the test tree may be narrowed while it exists
        # (#1322, decision item 2) — the arm-if-it-spells-`.py` policy that used to
        # live here is what rounds 3 and 4 both came through.
        assert decision.whole_tree, decision.notes
        assert any("cannot place" in note for note in decision.notes), decision.notes
        assert "tests/unit/test_scan_shape.py" in "\n".join(plan.report())
        return
    ...
    for forbidden in must_not:
        ...
        # says this reader observes a file it never opens, so it is checked through
        # the reader's own plan rather than through the selection.
...
def test_an_unplaceable_scan_stops_the_selection_and_says_so(tmp_path):
    """The rule is unconditional: the tool does not try to price the site.

    Deciding whether THIS site could reach THIS changed file is the shape
    enumeration that produced four rounds of false greens, so an unplaceable
    reference of any shape stops the test selection outright — and the line names
    the site, because a barrier a reader cannot see is the quiet narrowing this
    whole section exists to prevent.
    """
    root = _fixture_repo(tmp_path)
    _write(
        root,
        "tests/unit/test_stops.py",
        _SCAN_PROLOGUE
        + 'for path in Path(os.environ.get("SCAN_DIR", "tmp")).glob("*.py"):\n'
        + "    path.read_text()\n"
        + _CANNED_TEST,
    ...
    assert any("test_stops.py" in site for site in plan.unresolved), plan.unresolved
    assert plan.decisions["test"].whole_tree
    assert "cannot place" in report and "test_stops.py" in report


def test_the_barrier_reason_samples_the_sites_it_names(tmp_path):
    """Round 3: the reader that mattered hid behind a bare ellipsis.

    The count is always printed and the sample never silently truncates — a reader
    deciding whether to trust the plan needs to know how much it is not seeing.
    """
    root = _fixture_repo(tmp_path)
    ...
    assert len(plan.unresolved) >= 8
    assert "+2 more" in report or "more" in report, report
    assert "cannot place" in report


# ---------------------------------------------------------------------------
# The confidence rule: what may be narrowed, and what stops it
# ---------------------------------------------------------------------------


def test_a_boot_closure_change_is_a_barrier_and_an_outside_module_is_not(tmp_path):
    """The class that motivated the whole redesign, on both sides of its boundary.

    A boot-closure file is reachable by importing the APPLICATION, so the tests
    that can observe it are not a reference-graph question at all — 443 of the 503
    `local_operator/**` modules are inside that closure on this tree. The ~60
    outside it are where narrowing still pays, and this asserts both directions, so
    a future rule that simply refuses everything fails here.
    """
    root = _fixture_repo(tmp_path)
    _write(root, "local_operator/cli.py", "from local_operator.alpha import ALPHA\n")
    _write(root, "local_operator/outside.py", "OUTSIDE = 1\n")
    _write(root, "tests/unit/test_outside.py", "from local_operator.outside import OUTSIDE\n")

    boot = _plan(root, ["local_operator/alpha.py"])["test"]
    assert boot.whole_tree
    assert any("boot closure" in note for note in boot.notes), boot.notes

    outside = _plan(root, ["local_operator/outside.py"])["test"]
    assert not outside.whole_tree, outside.notes
    assert "tests/unit/test_outside.py" in outside.targets


def test_the_boot_closure_is_the_app_and_leaves_modules_outside_it():
    """Over the REAL tree: the closure exists, and it is not the whole package."""
    graph = ci_scope.build_import_graph(REPO)

    assert set(ci_scope.BOOT_ROOTS) <= graph.boot
    local = {rel for rel in graph.files if rel.startswith("local_operator/")}
    inside = graph.boot & local
    outside = local - graph.boot
    assert len(inside) > 200, "the app closure is not being computed"
    assert len(outside) > 20, "nothing is left outside the closure, so nothing can narrow"


def test_the_barrier_is_a_barrier_and_not_a_constant(tmp_path):
    """The dual of the real-tree guard: a resolvable tree still narrows."""
    root = _fixture_repo(tmp_path)
    graph = ci_scope.build_import_graph(root)
    universe = ci_scope._test_universe(graph, "tests/unit")

    assert not graph.unresolved_sites(), "the fixture is meant to be fully resolvable"
    assert ci_scope._test_barriers(graph, ["local_operator/alpha.py"], universe) == []


def test_no_changed_file_can_be_narrowed_while_the_graph_has_unresolved_references():
    """The property, asserted over the REAL tree rather than over a fixture.

    This is the shape the previous guard got wrong: it walked the resolver's own
    OUTPUT, so a spelling that never resolved was invisible to it. This one asks the
    barrier about every file the graph covers — so a rule that starts letting some
    class through while an unresolvable reference is standing fails here, whatever
    the class looks like.
    """
    graph = ci_scope.build_import_graph(REPO)
    universe = ci_scope._test_universe(graph, "tests/unit")

    assert graph.unresolved_sites(), "this tree has no unresolved site — re-derive this guard"
    narrowed = [
        rel for rel in sorted(graph.files) if not ci_scope._test_barriers(graph, [rel], universe)
    ]
    assert not narrowed, (
        f"{len(narrowed)} changed file(s) would narrow the test job while the graph "
        f"carries {len(graph.unresolved_sites())} unresolved reference(s): {narrowed[:5]}"
    )


#: Unresolved reference classes that must each stop the test selection. A table,
#: not a case per spelling: every one of rounds 1-4 arrived through a class that a
#: previous guard had enumerated INSTEAD of the property.
_UNRESOLVED_CLASSES: dict[str, str] = {
    "computed-import": (
        "import importlib\n\n" "def load(name):\n" "    return importlib.import_module(name)\n"
    ),
    "computed-path": (
    ...
    "pattern-held-in-a-variable": (
        "import glob\nfrom pathlib import Path\n\n"
        'PATTERN = "scripts/*.py"\n'
        "for name in glob.glob(PATTERN):\n"
        "    Path(name).read_text()\n"
    ),
    "wildcard-directory-in-the-pattern": (
        "import glob\nfrom pathlib import Path\n\n"
...
@pytest.mark.parametrize("label", sorted(_UNRESOLVED_CLASSES))
def test_every_unresolved_reference_class_stops_the_test_selection(tmp_path, label):
    """One row per class: whole-tree, with the class's own site named."""
    root = _fixture_repo(tmp_path)
    _write(root, "tests/unit/test_unresolved.py", _UNRESOLVED_CLASSES[label])

    plan = ci_scope.scope_plan(["test"], ["local_operator/alpha.py"], root)
    decision = plan.decisions["test"]

    assert plan.unresolved, label
