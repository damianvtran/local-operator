...
# ---------------------------------------------------------------------------
# The same measurement, stamped on the object the CALLER receives
# ---------------------------------------------------------------------------


def test_the_seam_stamps_the_window_onto_the_relayed_usage(tmp_path):
    """The band's source: the ``Usage`` the caller gets carries the window.

    The ledger and the status band must not disagree about the same call, and
    they cannot while both read one rule (``_decode_measure``). This is the
    relay half: the object the seam hands back — the provider's own ``Usage``
    instance, which the session turns into ``restored_usage`` and the frontend
    frame into ``last_usage`` — is stamped only when the call MEASURED a window.
    """
    store = AnalyticsStore(tmp_path / "a.db")
    rec = reset_recorder_for_test(store)
    fn = _fn("sess-1")
    deltas = [StreamTextDelta(delta=f"chunk {i} ") for i in range(6)]
    # Held in a local rather than read back off the event list: the list is
    # heterogeneous, so the element type carries no ``usage``.
    relayed = Usage(input_tokens=100, output_tokens=240, context_tokens=100)
    events = [
        *deltas,
        StreamUsageEvent(usage=relayed),
    ...
    agg, row = _decode_of(store)
    # The caller's copy and the ledger's row are the SAME measurement, which is
    # the property a second rule would break.
    assert relayed.decode_us == agg.decode_us > 0
    assert relayed.decode_tokens == agg.decode_tokens == 240
    rec.close()
    store.close()


def test_an_ineligible_call_leaves_the_relayed_usage_unstamped(tmp_path):
    """No window means NO FIELD, not a zero — the omission is the frame's budget.

    ``usage_components`` is a 200-cap list of this type, so a pair that is always
    present costs 48 B x 201 objects on the attach frame: the shape that took it
    over the 1 MiB socket line once already. The serializer drops the pair at 0,
    and this pins the other half of that bargain — the seam leaves 0 there for a
    call that measured nothing.
    """
    store = AnalyticsStore(tmp_path / "a.db")
    rec = reset_recorder_for_test(store)
    fn = _fn("sess-1")
    # ONE delta: a window needs two, and an unmeasured call must not be given one.
    relayed = Usage(input_tokens=100, output_tokens=9, context_tokens=100)
    events = [
        StreamTextDelta(delta="only"),
        StreamUsageEvent(usage=relayed),
        StreamEndEvent(stop_reason="stop", usage=relayed),
    ]
    asyncio.run(_drain(fn, _request(), events))
    ...
    assert agg.decode_calls == 0
    assert relayed.decode_us == 0 and relayed.decode_tokens == 0
    # And the wire says so by ABSENCE: the pair is dropped, not zeroed.
    dumped = relayed.model_dump(mode="json")
    assert "decode_us" not in dumped and "decode_tokens" not in dumped
    rec.close()
    store.close()
