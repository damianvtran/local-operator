...
class _FakeClassifier:
    """The seam double: ``recommend_resources``, that is all."""
...
@pytest.mark.asyncio
async def test_delivery_is_silent_and_never_consults_a_seam_that_offers_a_line() -> None:
    """The operator's requirement, pinned: the block IS the delivery.

    A recommendation is context for the model, not a report for the reader. The layer
    shipped a rendered one-liner from v0.57.0 to v0.59.8 and the operator ruled the
    surface out — a row of internal bookkeeping under every answer — so the renderer,
    the sink it painted through and the session's post-turn notice queue were removed
    rather than gated. This test is the fence around that: a seam which still
    publishes a ``notice()`` is never asked to render one, and the wiring advertises
    no channel to put a line on. Restoring the line is therefore a deliberate act
    (re-add the sink and the call site) rather than a config key someone flips back.
    """

    class _LoudSeam(_FakeClassifier):
        """The seam shape the layer shipped with: it CAN render a line."""

        def __init__(self, *args: Any, **kwargs: Any) -> None:
            super().__init__(*args, **kwargs)
            self.notice_calls: list[Any] = []

        def notice(self, recommendation: Any) -> str:
            self.notice_calls.append(recommendation)
            return "Suggestion added for this message: guide://tunnel"

    classifier = _LoudSeam(
        _Recommendation(
    ...
    assert not hasattr(hooks, "notice_sink"), "the wiring has no notice sink to deliver into"
    block = await session_factory._select_knowledge_block(hooks, OFF_QUERY, task_id="t1")

    assert "guide://tunnel" in block, "the delivery itself still happens"
    assert classifier.notice_calls == [], "a rendered line is never asked for"
    assert not hasattr(session_factory, "_emit_classification_notice")
    assert not hasattr(session_factory, "attach_classification_notices")
...
def _slow_classifier(recommendation: _Recommendation) -> _FakeClassifier:
    """A seam whose answer outlives the wait."""
...
async def test_an_answer_that_misses_the_wait_lands_in_the_same_turn() -> None:
    ...
    assert second.count("mcp://slack") == 1
    ...
    assert third.count("mcp://slack") == 1
...
async def test_an_answer_for_an_older_message_still_rides_the_next_one() -> None:
    ...
    to reach, so it is carried to the next admitted user message's prompt.
    """
    index = _FakeIndex(picked=[])
    classifier = _slow_classifier(_slack_recommendation())
    hooks = _hooks(index, classifier=classifier)
