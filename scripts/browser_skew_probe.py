        ...
        async with websockets.connect(
            ...
            while True:
                ...
                if method == "close":
                    ...
                    self.closed.append(str(message.get("params", {}).get("tab", "")))
                    await wire.send(
...
async def probe(version: str) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {"version": version, "port": port, "home": str(home)}
    try:
        ...
        peer = FakeExtension(port, version)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(asyncio.run(probe(args.version)), indent=2))
    return 0
