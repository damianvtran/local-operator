...
| `/model default` | Persists provider/model to config so later launches boot on it; bare `/model default` saves the model the session is already on, `/model default <p>/<id>` switches and saves, `/model saved` switches back to what is stored | `test_app_pilot` covers model switching plus the bare/explicit/`saved` spellings and the remote-session refusal |
...
| `hub` parent↔subagent channel | The parent lists every child it launched with `op='list'` — including finished, failed and paused ones whose job rows the manager has swept, each marked resumable or not — sends a note to one/several/`"all"` children, asks one a question and gets its answer back, steers one onto a new course, pauses one so it stays resumable, cancels one, and resumes a stopped/paused/failed one against its own transcript; a child answers (or speaks up unprompted) with the child-shaped `hub`. Notes/questions ride `Session.queue_aside`, steers ride `Session.steer`, pause rides `AsyncJobManager.cancel` plus recorded intent, resume rides transcript replay. | `tests/unit/harness/test_comms.py` (64: receipts and buffering, ask answered by the child's tool and by prose, narration-vs-answer, timeout, ask/cancel/pause/resume failure paths, roster status precedence and resumability, both tool shapes, plus 6 integration tests running real parent+child sessions through a scripted provider) |
| Subagent concurrency with the parent | A child launched by `task` keeps taking turns while the parent sits in a long `await` (i.e. inside a tool call), and the parent can put a question to a mid-tool-loop child and get its answer back without the child having to be idle. Locks in #133, which moved the per-turn compaction ruler (tiktoken) and the transcript writes off the shared event loop. | `tests/unit/harness/test_subagent_concurrency.py` (2, on real `Session` objects and the real `AsyncJobManager`; the first was mutation-checked — swapping its `await asyncio.sleep` for a synchronous `time.sleep` freezes the child and fails it) |
