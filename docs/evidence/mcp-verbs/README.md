...
| `typed_path_check.py` / `.txt` | Round-2 B1/B2: types every `/mcp <verb> ` transition with `pilot.press` per character and reports whether the picker actually OPENED, what it holds, and the destructive-gate verdict. This is the only harness here that exercises the `RefreshArgumentChoices` message; since #378 merged (`ae266b85`) it runs against this tree directly (`LOP_REPO=<repo root>`). |
...
`remove` frame also shows #378's ghost text, now that it is on main.
