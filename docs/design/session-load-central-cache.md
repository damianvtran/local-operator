...
- **One number in §2.4/§3(B) did not survive implementation**: `PAGE_CACHE_BYTES`
  was sized from serialized page sizes, while §2.4 specifies an accounting
  instrument that charges ~2x more. At 4 MiB accounted the cache admitted nothing
  at all for an ordinary conversation (measured; the shipped constant and the
  reason are in `session/page_cache.py`). The §3(B) paragraph that states the
  bound carries the correction inline.
...
above, never the 4 MiB this paragraph was drafted with. Note that the banner
names §2.4/§3(B): this sentence is the §3(B) one that states the bound.]**
`DesktopSessions` is one instance per app
